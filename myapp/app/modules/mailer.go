...
func (m *Mailer) do_config() {
	...
	if !ok {
		fmt.Println("1111111mail address not set")
	}
	m.port, ok = revel.Config.Int("mail.port")
	if !ok {
		fmt.Println("mail port not set")
	}
	m.from, ok = revel.Config.String("mail.from")
	if !ok {
		fmt.Println("mail.from not set")
	}
	m.username, ok = revel.Config.String("mail.username")
	if !ok {
		fmt.Println("mail.username not set")
	}
...
func (m *Mailer) getPassword() string {
	...
	fmt.Println("mail password not set")
	return ""
}
